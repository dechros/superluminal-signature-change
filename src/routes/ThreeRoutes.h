...
namespace slm
{
    ...
    public:
        ...
        /// The grid of matching requirements against descriptions, counted
        /// rather than asserted.
        ///
        /// The two indices do not enter the same way, and the counts are what
        /// show it. A matching requirement decides whether a crossing carrying
        /// no returning wave exists at all, so it governs whether the cell
        /// holds a journey. A description decides which duration is read off,
        /// so it governs what the cell reports. Neither index touches the
        /// other's job: the transmitted phase is built from the interior
        /// wavenumber and the thickness, and no matching requirement appears in
        /// it. So the grid factorises, and the number of genuinely distinct
        /// answers is far below the number of cells.
        struct Grid
        {
