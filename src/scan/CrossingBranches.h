...
namespace slm
{
    ...
    /// that carries the near-side time.
    ///
    ...
    /// and all eight send the near-side time slot to the same single axis on the far side.
    ...
    ///
    /// The two families differ by reversing the near-side time axis, which is an
    /// isometry of the near-side metric. Anything that selects between them therefore lies
    ...
    public:
        ...
        /// Reversal of the near-side time axis together with the far side's distinguished
        ...
        /// along the far-side space direction and arrive the other way along the near-side time.
        ...
        /// both sides rather than only on the near one.
        static bool isIsometryOfBothRegions(const Matrix4 &map);

        /// Displacement produced on the near-side clock by a unit far-side displacement,
        /// for the given branch. Its sign is the branch's time sign.
        static double nearTimeFromUnitFarSide(const Branch &branch);
    };

    /// Section computing whether the admissible crossings fix the direction of
    /// the displacement they produce on the near-side clock.
